#!/usr/bin/env python3
"""
Raspberry Pi 4 Face Recognition System - Full Screen Edition
- Full resolution capture (3280×2464)
- Full screen camera preview on TFT
- Minimal UI, CLI-based control
"""
...
PREVIEW_WIDTH = 320
PREVIEW_HEIGHT = 240
CAPTURE_WIDTH = 3280    # FULL RESOLUTION!
CAPTURE_HEIGHT = 2464   # FULL RESOLUTION!
MIN_DETECTION_CONFIDENCE = 0.5

# UI Colors
COLOR_BOX = (0, 255, 0)      # Green box for detected faces
...
# ========================================
class TFTDisplay:
    """ILI9341 TFT Display - Full Screen Mode"""
    ...
    def show_frame(self, frame, faces=[], status=None, fps=0):
        """
        Display full-screen camera feed with minimal overlay.
        frame: RGB numpy array (320×240)
        ...
        # Draw overlays directly on image
        ...
        # Minimal status overlay (top-left corner)
        if status or fps > 0:
            # Semi-transparent background for text
            overlay_text = f"FPS:{fps:.0f}"
            if status:
                overlay_text += f" | {status}"

            # Small text in corner
            draw.text((2, 2), overlay_text, font=self.font_small, fill=COLOR_TEXT)
...
# ========================================
# Camera - Full Resolution Capture
# ========================================
class OptimizedPiCamera:
    ...
    def start(self):
        ...
        try:
            ...
            # Preview config: 320×240 for fast display (Mode 2: 1640×1232 sensor)
            self.preview_config = self.picam2.create_preview_configuration(
                main={"size": (PREVIEW_WIDTH, PREVIEW_HEIGHT), "format": "RGB888"},
                raw={"size": (1640, 1232)}  # Mode 2: Fast, full FOV
            )

            # Capture config: FULL RESOLUTION for embeddings (Mode 0: 3280×2464)
    ...
    def read_preview(self):
        """Fast preview frame for display"""
        try:
    ...
    def capture_high_res(self):
        ...
        try:
            ...
            self.picam2.start()
            time.sleep(0.8)  # Longer wait for full resolution
...
def capture_face_embedding(camera, detector, display):
    """Capture face with 3-second countdown"""
    ...
    while countdown > 0:
        ...
        # Show full-screen preview with countdown
...
def main():
    ...
    print("  'v' - VERIFY face against database")
            ...
            if cmd == 'a':
                ...
                if embedding is not None:
                    ...
                    if name:
                        ...
                        print("❌ No name entered, face not saved")
                print("="*60 + "\n")
                ...
                if embedding is not None:
                    ...
                    if best_score > SIMILARITY_THRESHOLD:
                        ...
                        print(f"   Confidence: {best_score:.3f} (threshold: {SIMILARITY_THRESHOLD})")
                    else:
                        print(f"❌ ACCESS DENIED")
                        print(f"   Best match: {best_name}")
                        print(f"   Score: {best_score:.3f} (threshold: {SIMILARITY_THRESHOLD})")
                    print("-"*60)
                print("="*60 + "\n")
