...
def face_worker_process(cmd_q: Queue, res_q: Queue, busy_evt: Event,
    ...
    Autonomous mode: continuous detect-and-identify whenever a face is in FOV.
    Command mode   : honour requests from server (enroll / verify / delete / list).
    """
    _pin(CORE_FACE)
    ...
    try:
        while True:
            # ── Priority: check for incoming command ──────────────────────────
            if not cmd_q.empty():
            ...
            # ── Autonomous: read a preview frame and run detection ────────────
            ...
            # Autonomous identification: if a face is detected and DB is ready
            if detects and len(db) >= MIN_GALLERY_SIZE and not busy_evt.is_set():
                probe = capture_probe(camera, detector, display)
...
def voice_worker_process(cmd_q: Queue, res_q: Queue, busy_evt: Event,
    ...
    Autonomous mode: blocking UART listen → identify on every incoming audio session.
    Command mode   : enroll / verify / delete / list.
    """
    _pin(CORE_VOICE)
...
def main():
    ...
    log.info("=" * 70)
    log.info("  Biometric Orchestrator  v2.0")
